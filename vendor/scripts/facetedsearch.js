;(function(){
  ...
  var defaults = {
    ...
    paginationCount    : 20,
    callbackResultUpdate: dummy,
    callbackUiCreated: dummy,
    callbackFacetedsearchresultupdate: dummy
}
    ...
    settings.currentResults = _.select(settings.items, function(item) {
      ...
      if(settings.textFilter) {
        ...
        if(filterValue) {
          ...
          _.each(split, function(term) {
            if(itemString.indexOf(term) == -1) filtersApply = false;
          });
        }
      }
      return filtersApply;
  ...
  function createFacetUI() {
    ...
    _.each(settings.facets, function(facettitle, facet) {
      ...
      facetlist.append("<option value='' selected>-</option>");
    ...
    var deselect = $(settings.deselectTemplate).click(function(event){
      ...
      var facetlist = $(settings.facetListContainer);
      //facetlist.triggerChange();
      $.facetUpdate();
    });
    $(bottom).append(deselect);
    $(settings.facetSelector).trigger("facetuicreated");
    settings.callbackUiCreated(settings.currentResults);
  }
  ...
  function showMoreResults() {
    ...
    settings.callbackFacetedsearchresultupdate(settings.currentResults);
  }
